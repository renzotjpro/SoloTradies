...
export function InvoicePreview({ invoiceData }: { invoiceData?: InvoiceData }) {
  ...
  const accentColor = invoiceData?.accentColor ?? s.colour_graphical;
  ...
  const resolvedHeaderLayout = invoiceData?.headerLayout ?? s.header_layout;
  ...
  const isCentred = resolvedHeaderLayout === "centred";
  const isSplit = resolvedHeaderLayout === "split";
    ...
    >
      ...
      {isCentred && (
        <div className="px-7 pt-6 pb-4 text-center border-b border-gray-100">
          <div
            className="inline-block px-4 py-1.5 rounded font-bold tracking-wider text-[1.3em] mb-3"
            style={{ backgroundColor: accentColor, color: "white" }}
          >
            {L.invoiceTitle}
          </div>
          <div className="font-bold text-[1.5em]" style={{ color: textColor }}>
            {s.business_name || s.display_name || "Your Business Name"}
          </div>
          {s.abn && <div className="text-[0.85em] text-gray-400 mt-0.5">ABN: {s.abn}</div>}
          {s.phone && <div className="text-[0.85em] text-gray-500">{s.phone}</div>}
          {s.email && <div className="text-[0.85em] text-gray-500">{s.email}</div>}
        </div>
      )}
      ...
      {isSplit && (
        <div className="px-7 pt-6 pb-4 flex items-start justify-between border-b border-gray-100">
          <div>
            ...
            {s.abn && <div className="text-[0.85em] text-gray-400 mt-0.5">ABN: {s.abn}</div>}
            {s.address && (
              <div className="text-[0.85em] text-gray-500 mt-1 whitespace-pre-line">{s.address}</div>
            )}
            {s.phone && <div className="text-[0.85em] text-gray-500">{s.phone}</div>}
            {s.email && <div className="text-[0.85em] text-gray-500">{s.email}</div>}
          </div>
          <div
            className="font-bold tracking-wider text-[1.3em] px-3 py-1 rounded"
            style={{ backgroundColor: accentColor, color: "white" }}
          >
            {L.invoiceTitle}
          </div>
      ...
      <div className="px-7 py-5 space-y-4">
        ...
        <div className="flex justify-between gap-4">
          ...
          <div className="flex-1">
            <div className="text-[0.85em] font-semibold uppercase tracking-wider text-gray-400 mb-1">
              {L.billTo}
            </div>
            <div className="font-semibold text-[1.05em]">{data.clientName}</div>
            {data.clientCompany && (
              <div className="text-[0.85em] text-gray-500">{data.clientCompany}</div>
            )}
            {s.show_client_address && (
              <div className="text-[0.85em] text-gray-400 mt-0.5">123 Client St, Melbourne VIC 3000</div>
            )}
            {s.show_client_abn && (
              <div className="text-[0.85em] text-gray-400">ABN: 12 345 678 901</div>
            )}
          </div>
          ...
          <div className="text-right space-y-1 shrink-0">
            {[
              ...
              <div key={label} className="flex items-baseline justify-end gap-3">
                <span className="text-[0.85em] text-gray-400">{label}</span>
                <span className="text-[0.95em] font-semibold min-w-[70px] text-right">{val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-2">
          {/* Table header */}
          <div
            className="grid text-[0.85em] font-semibold uppercase tracking-wide py-1.5 px-2 rounded-sm"
            style={{
          ...
          {data.items.map((item, i) => (
            <div
              key={i}
              className="grid items-center py-2 px-2 text-[0.9em]"
              style={{
                gridTemplateColumns: s.show_quantity_column
                  ? "1fr 55px 65px 55px 65px"
                  : "1fr 65px 55px 65px",
        ...
        <div className="flex justify-end mt-1">
          <div className="w-48 space-y-1">
            {s.show_gst_breakdown && (
              <>
                <div className="flex justify-between text-[0.9em] text-gray-500">
            ...
            <div
              className="flex justify-between font-bold py-1.5 px-2 rounded text-[1.05em]"
        ...
        {s.show_payment_details && (
          <div className="pt-3 border-t border-gray-100">
            <div
              className="text-[0.85em] font-bold uppercase tracking-wider mb-1.5"
            ...
            {s.footer_layout === "two_column" ? (
              <div className="grid grid-cols-2 gap-4">
                <div className="text-[0.8em] text-gray-500 whitespace-pre-line leading-relaxed">
                  {s.payment_details || "Bank transfer details here"}
                </div>
                <div className="text-[0.8em] text-gray-500 leading-relaxed">
        ...
        {s.show_footer_message && s.footer_message && (
          <div className="pt-3 border-t border-gray-100 text-[0.8em] text-gray-400 italic text-center leading-relaxed whitespace-pre-line">
            {s.footer_message}
          </div>
        )}
        ...
        {s.show_terms_conditions && s.terms_conditions && (
          <div className="pt-3 border-t border-gray-100">
            <div className="text-[0.75em] font-semibold text-gray-400 uppercase tracking-wide mb-1">
              Terms & Conditions
            </div>
            <div className="text-[0.7em] text-gray-400 leading-relaxed whitespace-pre-line">
              {s.terms_conditions}
            </div>
          </div>
        )}
