...
def read_invoices(skip: int = 0, limit: int = 100, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
    invoices = crud.get_invoices(sb, owner_id=owner_id, skip=skip, limit=limit)
    return invoices

@router.get("/{invoice_id}", response_model=schemas.Invoice)
def read_invoice(invoice_id: int, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
...
def create_invoice(invoice: schemas.InvoiceCreate, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
    return crud.create_invoice(sb=sb, invoice=invoice, owner_id=owner_id)

@router.put("/{invoice_id}", response_model=schemas.Invoice)
def update_invoice(invoice_id: int, invoice: schemas.InvoiceUpdate, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
...
def update_invoice_status(invoice_id: int, body: schemas.InvoiceStatusUpdate, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
...
def delete_invoice(invoice_id: int, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
...
def download_invoice_pdf(invoice_id: int, sb: Client = Depends(get_supabase), owner_id: str = Depends(get_current_user)):
    invoice = crud.get_invoice(sb, invoice_id=invoice_id, owner_id=owner_id)
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
